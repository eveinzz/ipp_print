...
| `print-quality` job template（type2 enum：3=draft / 4=normal / 5=high）；与 `printer-resolution` 冲突时打印机 SHOULD 以 `print-quality` 为准 | RFC 8011 | §5.2.13、Table 12 |
...
**0.7.0 起 CORE FREEZE**——内核契约（Contract v2）封板；0.7.x 只接受修 BUG、
协议正确性、兼容性、性能、测试与平台适配。后续增强（流式发现、诊断）为
additive-only 候选，见 [TODO.md](TODO.md)；已发布变更记录于
